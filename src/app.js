...
function showWeather(response) {
  ...
  for(let i=0; i=6; i++){
    document.querySelector("#first-hour-temp").innerHTML = `${Math.round(response.data.list[i].main.temp)}℃`;
  }
  /*document.querySelector("#first-hour-temp").innerHTML = `${Math.round(response.data.list[0].main.temp)}℃`;
    ...
    for(let i=0; i=6; i++){
      document.querySelector("#two-hour").innerHTML = forecastHour(response.data.list[i].dt * 1000); 
    }
